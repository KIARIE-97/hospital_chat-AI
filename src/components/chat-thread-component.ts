...
export class ChatThreadComponent extends LitElement {
					...
					${this.actionButtons.map(
						(actionButton) => html`
							<chat-action-button
								...
								@click="${(event) =>
									this.actionButtonClicked(actionButton, entry, event)}"
							></chat-action-button>
						`
					)}
					<chat-action-button
						.label="${globalConfig.COPY_RESPONSE_BUTTON_LABEL_TEXT}"
						.svgIcon="${this.isResponseCopied
							? iconSuccess
							: iconCopyToClipboard}"
	...
	renderTextEntry(textEntry: ChatMessageText) {
		// If the value is empty, show a placeholder for bot messages
		if (!textEntry.value || textEntry.value.trim() === "") {
			return html`<p class="chat__txt--entry chat__txt--empty">
				No response from AI.
			</p>`;
		}
		const entries = [
			html`<p class="chat__txt--entry">${unsafeHTML(textEntry.value)}</p>`,
		];
							...
							(followupQuestion) => html`
								<li class="items__listItem--followup">
									<a
										...
										@click="${(event) =>
											this.handleFollowupQuestionClick(
												followupQuestion,
												entry,
												event
											)}"
	...
	override render() {
		// Debug: Log the chatThread and error states before rendering
		console.log(
			"[chat-thread-component] render() called. chatThread:",
			this.chatThread
		);
		if (this.chatThread.some((msg) => msg.error)) {
			console.warn(
				"[chat-thread-component] At least one message has an error:",
				this.chatThread.filter((msg) => msg.error)
			);
		}
		return html`
			<ul class="chat__list" aria-live="assertive">
				${this.chatThread.map(
					(message) => html`
						<li
							class="chat__listItem ${message.isUserMessage
								? "user-message"
								: ""}"
						>
							<div
								class="chat__txt ${message.isUserMessage ? "user-message" : ""}"
							>
								${message.isUserMessage
									? ""
									: this.renderResponseActions(message)}
								${message.text.map((textEntry) =>
									this.renderTextEntry(textEntry)
								)}
