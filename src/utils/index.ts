...
export function processText(
	...
	const findFollowingSteps =
		/:(.*?)(?:Follow-up questions:|Next questions:|<<|$)/s;
	...
	const followingStepsText = followingStepsMatch
		? followingStepsMatch[1].trim()
		: "";
	...
	const nextRegex = hasNextQuestions
		? findNextQuestions
		: findQuestionsbyDoubleArrow;
	...
	replacedText =
		stepsIndex !== -1 ? inputText.substring(0, stepsIndex + 6) : inputText;
...
export function cleanUpFollowUp(followUpList: string[]): string[] {
	if (
		...
		followUpList[0].startsWith("<<")
	) {
		followUpList = followUpList.map((followUp) =>
			followUp.replace("<<", "").replace(">>", "")
		);
	}
	return followUpList;
}
